...
if (typeof studentInformation.name === "string") {
    console.log(studentInformation);
}
else {
    console.log("only use alphbets");
}
const studentID = Math.floor(Math.random() * 100000);
console.log(chalk.blue(`\n\nYour Name is ${chalk.magentaBright.bold(studentInformation.name)}. You are selecting to join ${chalk.magentaBright.bold(studentInformation.course)} course.
...
if (idAns.id === studentID) {
    ...
    if (answer.fee === "Fee deposite") {
        ...
        if (myBalance >= depositeAmnt.deposite) {
            ...
            console.log(chalk.red(`Your current balance is ${chalk.green.bold(myBalance)}`));
        }
        else {
